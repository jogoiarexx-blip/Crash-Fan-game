(()=>{
const c=document.getElementById('game'),x=c.getContext('2d');x.imageSmoothingEnabled=false;const W=c.width,H=c.height;const GAME_VERSION='1.16';
...
let playerImg=null,crouchImg=null,akuImg=null,aku3PlayerImg=null,tntImg=null,bounceBoxImg=null,lifeBoxImg=null,crystalImg=null,bgImg=null,trapImg=null,portalImg=null,checkpointImg=null,soulImg=null,tileImg=null,turtleImg=null,armadilloImg=null,magmaBeetleImg=null,emberBatImg=null,snowballImg=null,bearRideImg=null,boulderImg=null,boarRideImg=null,roadObstaclesImg=null,bossGuardianImg=null,spikeAssetImg=null,floorSpikeAssetImg=null,logAssetImg=null,pressAssetImg=null,poisonAssetImg=null,iceSpikeAssetImg=null,ruinEndImg=null,menuLogoImg=null,levelSelectBgImg=null,biomeTileImg=null,biomeEnemyImg=null;
...
class LevelManager{
 ...
 cleanupRuntime(reason='transition'){
  ...
  bgImg=null;biomeTileImg=null;biomeEnemyImg=null;snowballImg=null;bearRideImg=null;boulderImg=null;boarRideImg=null;roadObstaclesImg=null;
 ...
 async load(level,done,{forceReload=false}={}){
  ...
  state='loading';loadingTargetLevel=level;loadingProgress=0;loadingError='';loadingErrorDetails='';loadingRetry=null;
...
function blankLevelRecord(){return{completed:false,gem:false,crystal:false,bestTime:null,bestScore:0,bestBoxes:0,totalBoxes:0,bestDeaths:null,bestHits:null,bestCombo:0,nearMisses:0}}
...
function beginDeath(msg='Crash caiu!',kind='impact'){if(deathAnim>0||state!=='play')return;akuInvT=0;akuEquipT=0;akuTrail.length=0;if(aku===3)aku=0;stopInvincibleMusic();playSfx('death',1,kind==='freeze'?.82:1,.2);deaths++;lives--;deathX=p.x;deathY=Math.max(36,Math.min(p.y,groundY-p.h));deathFacing=p.facing||1;deathKind=kind;deathAnim=DEATH_DURATION;p.vx=p.vy=0;p.spin=0;shake=kind==='explosion'||kind==='snowball'?.62:.46;toast=msg;toastT=1.0}
...
function lose(force=false,kind='impact'){if((inv>0&&!force)||deathAnim>0)return;if(!force&&akuInvT>0){shake=Math.max(shake,.08);return}if(aku>0&&aku<3){playSfx('hurt',1.08,.9,.15);aku--;inv=2;shake=.25;akuProtectT=.65;toast='Aku Aku protegeu você!';toastT=1.7;return}beginDeath(DEATH_TITLES[kind]||'Crash perdeu uma vida!',kind)}
...
function updateBossLevel(dt){
 ...
 window.FinalBoss.update(bossState,dt,{player:{x:p.x,y:p.y,w:p.w,h:p.h},attackRect:p.spin>0?spinAttackRect():null,ground:groundY,
  ...
  hitBoss:hp=>{score+=1200;shake=Math.max(shake,.42);playLoadedSfx('bossHit',1,1);toast='ACERTOU!  VIDA DO GUARDIÃO: '+hp+'/9';toastT=1.05},
...
function rideProjection(lane,d){const t=Math.max(0,Math.min(1,1-d/980)),curve=Math.sin((rideState?.distance||0)/850)*36*(d/980);return{x:W/2+lane*(42+170*t)+curve,y:188+t*t*330,scale:.18+t*.94}}
function avalancheProjection(lane,d){const a=avalancheState||buildAvalancheState(),t=Math.max(0,Math.min(1,1-d/980)),curve=Math.sin(a.distance/760)*30*(d/980);return{x:W/2+lane*(40+176*t)+curve,y:186+t*t*332,scale:.17+t*.96}}
...
function drawRoadObstacle(o,pr,s,jungle){
 ...
 const wide=o.type==='ice'||o.type==='crate',w=(wide?132:116)*s,h=(wide?108:104)*s;
 ds(roadObstaclesImg,roadObstacleSource(o,jungle),pr.x-w/2,pr.y-h,w,h,false,1);return true;
}
...
function drawRideLevel(){
 ...
 x.save();if(!reducedMotion&&graphicsTier()>0){x.translate(W/2,H/2);x.rotate(rs.lean*.012);x.translate(-W/2,-H/2)}x.fillStyle=jungle?'rgba(46,105,48,.30)':'rgba(220,246,255,.38)';x.fillRect(0,160,W,380);
 x.fillStyle=jungle?'#a77c3e':'#d8f3ff';x.beginPath();x.moveTo(390,185);x.lineTo(570,185);x.lineTo(900,H);x.lineTo(60,H);x.closePath();x.fill();x.strokeStyle=jungle?'#4e672e':'#75bad5';x.lineWidth=5;x.stroke();
 for(let d=80;d<980;d+=graphicsTier()?120:180){const a=rideProjection(-.5,d),b=rideProjection(-.5,d+54),c=rideProjection(.5,d),e=rideProjection(.5,d+54);x.globalAlpha=.42;x.strokeStyle='#539fbe';x.lineWidth=Math.max(1,6*a.scale);x.beginPath();x.moveTo(a.x,a.y);x.lineTo(b.x,b.y);x.moveTo(c.x,c.y);x.lineTo(e.x,e.y);x.stroke()}
 x.globalAlpha=1;for(let i=0;i<fxCount(8,13,18);i++){const d=((i*127-rs.distance*.72)%980+980)%980,side=i%2?-1:1,pr=rideProjection(side*1.52,d);x.fillStyle=jungle?(i%3?'#2c7a39':'#4f9b3f'):'#edfaff';x.beginPath();x.moveTo(pr.x,pr.y);x.lineTo(pr.x-side*18*pr.scale,pr.y+48*pr.scale);x.lineTo(pr.x+side*20*pr.scale,pr.y+48*pr.scale);x.closePath();x.fill();x.fillStyle=jungle?'#51351e':'#2d6682';x.fillRect(pr.x-side*3*pr.scale,pr.y+42*pr.scale,6*pr.scale,30*pr.scale)}
 ...
 const laneX=rs.lane*125,air=rs.y*.65,runFr=Math.floor(p.anim)%4,frame=rs.y>22?6:Math.abs(rs.targetLane-rs.lane)>.08?(rs.targetLane<rs.lane?4:5):runFr;
 if(introT>0&&!reducedMotion)drawPolarMountIntro();
 else if(deathAnim>0){const q=(DEATH_DURATION-deathAnim)/DEATH_DURATION,bounce=Math.sin(Math.min(1,q*1.45)*Math.PI)*72;x.save();x.translate(W/2+laneX+q*54,462-air-bounce+q*86);x.rotate(q*3.15);x.scale(1+q*.08,1-q*.12);ds(mount,[7*256,0,256,256],-125,-205,250,250,false,Math.max(.12,1-q*.76));x.restore();x.save();x.fillStyle=jungle?'#cfac6d':'#effcff';for(let i=0;i<fxCount(3,6,9);i++){x.globalAlpha=Math.max(0,.65-q*.55);x.beginPath();x.arc(W/2+laneX-45+i*13-q*55,464+i%2*9,4+i%3*2,0,Math.PI*2);x.fill()}x.restore()}else ds(mount,[frame*256,0,256,256],W/2-125+laneX,255-air,250,250);
 x.restore();
 x.save();x.fillStyle='#06141de8';x.strokeStyle='#8fdcf0';x.lineWidth=2;x.beginPath();x.roundRect(14,14,700,62,12);x.fill();x.stroke();x.fillStyle='#fff';x.font='bold 18px Arial';x.fillText('VIDAS × '+lives,34,42);x.fillText('FRUTAS '+fruits,170,42);x.fillText('VELOCIDADE '+Math.round(rs.speed),315,42);x.fillStyle='#9bdff2';x.font='bold 12px Arial';x.fillText('DISTÂNCIA',515,31);x.fillStyle='#173746';x.fillRect(515,42,170,13);x.fillStyle='#70ddf4';x.fillRect(515,42,170*Math.min(1,rs.distance/RIDE_LENGTH),13);x.restore();
 drawSpecialCombo(rs);if(toastT>0){x.textAlign='center';x.font='bold 20px Arial';x.fillStyle='#00131dee';x.fillRect(W/2-220,95,440,38);x.fillStyle='#fff';x.fillText(toast,W/2,121);x.textAlign='left'}drawCinematicOverlay();drawDeathCinematicOverlay();if(paused)drawPauseMenu();
}
function drawAvalancheLevel(){
 ...
 x.save();if(!reducedMotion&&graphicsTier()>0){x.translate(W/2,H/2);x.rotate(a.lean*.013+(danger>.72?Math.sin(swingClock*15)*.003:0));x.translate(-W/2,-H/2)}x.fillStyle=jungle?'rgba(49,107,42,.30)':'rgba(218,244,255,.40)';x.fillRect(0,154,W,H-154);x.fillStyle=jungle?'#9a713a':'#d9f4ff';x.beginPath();x.moveTo(392,184);x.lineTo(568,184);x.lineTo(918,H);x.lineTo(42,H);x.closePath();x.fill();x.strokeStyle=jungle?'#49662c':'#72bad7';x.lineWidth=5;x.stroke();
 for(let i=0;i<fxCount(9,14,19);i++){const d=((i*137-a.distance*.82)%980+980)%980,side=i%2?-1:1,pr=avalancheProjection(side*1.5,d);x.fillStyle=jungle?(i%3?'#287538':'#4b973e'):'#f2fbff';x.beginPath();x.moveTo(pr.x,pr.y);x.lineTo(pr.x-side*18*pr.scale,pr.y+48*pr.scale);x.lineTo(pr.x+side*20*pr.scale,pr.y+48*pr.scale);x.closePath();x.fill();if(jungle){x.fillStyle='#4a301b';x.fillRect(pr.x-side*3*pr.scale,pr.y+42*pr.scale,6*pr.scale,28*pr.scale)}}
 ...
 if(deathAnim>0){const q=(DEATH_DURATION-deathAnim)/DEATH_DURATION;x.save();x.translate(W/2+laneX+Math.sin(q*18)*18,500-air+q*42);x.rotate(q*6.2);x.scale(1+q*.34,Math.max(.3,1-q*.66));ds(playerImg,q<.3?sr.hurt:sr.spin2,-62,-158,124,158,flip,Math.max(.08,1-q*.82));x.restore()}else{x.save();x.translate(W/2+laneX,500-air+bob);x.rotate((a.targetLane-a.lane)*-.16);const squash=a.y>0?1.06:.98+Math.sin(p.anim*Math.PI*2)*.025;x.scale(1/squash,squash);ds(playerImg,run,-62,-158,124,158,flip,1);x.restore()}
 for(let i=0;i<fxCount(0,6,12)&&!reducedMotion;i++){const xx=(i*83+a.distance*1.7)%W,yy=210+(i*47+a.distance*1.2)%310;x.globalAlpha=.18+danger*.28;x.strokeStyle=jungle?'#d9bd6a':'#effcff';x.lineWidth=2+i%3;x.beginPath();x.moveTo(xx,yy);x.lineTo(xx+(xx-W/2)*.08,yy+35+danger*30);x.stroke()}x.globalAlpha=1;x.restore();
 x.save();x.fillStyle='#06141de8';x.strokeStyle=danger>.68?'#ff795f':jungle?'#9fd56f':'#8fdcf0';x.lineWidth=2;x.beginPath();x.roundRect(14,14,930,62,12);x.fill();x.stroke();x.fillStyle='#fff';x.font='bold 18px Arial';x.fillText('VIDAS × '+lives,34,42);x.fillText('FRUTAS '+fruits,170,42);x.fillText('VELOCIDADE '+Math.round(a.speed),305,42);x.fillStyle=jungle?'#b9e58f':'#9bdff2';x.font='bold 12px Arial';x.fillText('DISTÂNCIA',485,31);x.fillStyle='#173746';x.fillRect(485,42,170,13);x.fillStyle=jungle?'#8ecb55':'#70ddf4';x.fillRect(485,42,170*Math.min(1,a.distance/AVALANCHE_LENGTH),13);x.fillStyle=danger>.68?'#ff795f':jungle?'#b9e58f':'#9bdff2';x.fillText(jungle?'PEDRA GIGANTE':'AVALANCHE',690,31);x.fillStyle='#173746';x.fillRect(690,42,220,13);x.fillStyle=danger>.68?'#ff5b4b':jungle?'#9bcf65':'#9beaff';x.fillRect(690,42,220*danger,13);x.restore();
 drawSpecialCombo(a);if(toastT>0){x.textAlign='center';x.font='bold 20px Arial';x.fillStyle='#00131dee';x.fillRect(W/2-220,94,440,38);x.fillStyle='#fff';x.fillText(toast,W/2,120);x.textAlign='left'}drawCinematicOverlay();drawDeathCinematicOverlay();if(paused)drawPauseMenu();
}
...
function drawBossLevel(){
 ...
 const stageColor=s.phase===1?'rgba(42,145,65,.14)':s.phase===2?'rgba(226,82,24,.16)':'rgba(75,190,255,.16)';x.fillStyle=stageColor;x.fillRect(0,0,W,H);x.fillStyle='rgba(4,8,13,.20)';x.fillRect(0,0,W,H);x.fillStyle='#382719';x.fillRect(0,groundY,960,H-groundY);x.fillStyle=s.phase===1?'#77c956':s.phase===2?'#ff8447':'#86ddff';x.fillRect(0,groundY,960,7);
 for(const q of s.hazards){const warning=q.t>q.active,top=groundY-(q.kind==='ice'?115:70);x.save();x.globalAlpha=warning?.35:.88;x.fillStyle=warning?'#ffe36b':q.kind==='ice'?'#8eeaff':'#ff6b32';x.fillRect(q.x,warning?groundY-9:top,q.w,warning?9:groundY-top);if(!warning){x.fillStyle=q.kind==='ice'?'#efffff':'#ffd35a';for(let i=0;i<3;i++){x.beginPath();x.moveTo(q.x+i*q.w/3,groundY);x.lineTo(q.x+(i+.5)*q.w/3,top-(i%2)*13);x.lineTo(q.x+(i+1)*q.w/3,groundY);x.closePath();x.fill()}}x.restore()}
 ...
 if(bossGuardianImg){const fr=window.FinalBoss.sourceFrame(s);x.save();if(s.flash>0)x.globalAlpha=.62;ds(bossGuardianImg,fr,s.x-20,s.y,s.w+40,s.h);x.restore()}
 ...
 if(deathAnim>0)drawPlatformDeath(DEATH_DURATION-deathAnim,p.x+p.w/2,p.y+p.h,p.facing<0);else ds(playerImg,pf,p.x-21,p.y-30,88,108,p.facing<0,inv>0&&Math.floor(inv*12)%2?.42:1);
 x.restore();
 const stageName=s.phase===1?'SELVA ANCESTRAL':s.phase===2?'CÂNION ARDENTE':'GELO ETERNO';x.save();x.fillStyle='#07151de8';x.strokeStyle='#e8c058';x.lineWidth=3;x.beginPath();x.roundRect(180,16,600,70,14);x.fill();x.stroke();x.textAlign='center';x.fillStyle='#fff3bf';x.font='900 18px Arial';x.fillText('GUARDIÃO DAS NOVE GEMAS  •  '+stageName,W/2,42);x.fillStyle='#351925';x.fillRect(254,55,452,17);x.fillStyle=s.phase===1?'#7ee178':s.phase===2?'#ff9b45':'#8fdcff';x.fillRect(258,59,444*(s.hp/s.maxHp),9);x.fillStyle='#fff';x.font='bold 12px Arial';x.fillText(s.hp+' / '+s.maxHp,W/2,70);x.restore();
 if(s.vulnerable>0&&!s.defeated){x.save();x.textAlign='center';x.font='900 18px Arial';x.fillStyle='#ffe269';x.fillText('NÚCLEO VULNERÁVEL — USE O GIRO!',W/2,116);x.restore()}
 if(s.attack&&!s.attack.spawned){const names={slam:'IMPACTO NO CHÃO!',gem:'RAJADA DE GEMAS!',fire:'CHAMAS DO CÂNION!',ice:'GELO ANCESTRAL!'};x.save();x.textAlign='center';x.font='900 22px Arial';x.fillStyle='#ffef96';x.fillText(names[s.attack.kind]||'ATAQUE!',W/2,126);x.restore()}
 ...
 if(toastT>0){x.save();x.textAlign='center';x.fillStyle='#00131dee';x.fillRect(W/2-275,405,550,36);x.fillStyle='#fff';x.font='bold 17px Arial';x.fillText(toast,W/2,429);x.restore()}drawDeathCinematicOverlay();if(paused)drawPauseMenu();
}
function draw(){if(levelMode==='boss'){drawBossLevel();return}if(levelMode==='ride'){drawRideLevel();return}if(levelMode==='avalanche'){drawAvalancheLevel();return}bg();x.save();let sx=shake>0?(Math.random()-.5)*12:0,sy=shake>0?(Math.random()-.5)*8:0;x.translate(-Math.round(camX)+Math.round(sx),Math.round(sy));
 ...
 if(toastT>0){x.font='bold 20px Arial';let tw=x.measureText(toast).width+30;x.fillStyle='#00131dee';x.fillRect(W/2-tw/2,98,tw,38);x.fillStyle='#fff';x.fillText(toast,W/2-tw/2+15,124)}drawCinematicOverlay();drawDeathCinematicOverlay();if(paused)drawPauseMenu();
}
...
function drawPauseMenu(){uiButtons=[];x.fillStyle='#00110cd8';x.fillRect(0,0,W,H);x.textAlign='center';x.fillStyle='#ffd15b';x.font='bold 40px Arial';x.fillText('PAUSADO',W/2,58);x.fillStyle='#d8f1df';x.font='14px Arial';x.fillText(currentLevelName+' • SLOT '+(activeSlot||'—'),W/2,80);panel(295,91,370,444,.95);const opts=[['CONTINUAR','Voltar ao jogo'],['SALVAR JOGO','Salvar no Slot '+(activeSlot||'—')],['MÚSICA '+audioPct('music')+'%','ENTER/←→ altera'],['EFEITOS '+audioPct('sfx')+'%','ENTER/←→ altera'],['GRÁFICOS '+graphicsLabel(),'ENTER/←→ altera'],['CONTROLES','Ver configuração atual'],['REINICIAR FASE','Recomeçar a fase'],['MENU PRINCIPAL','Salvar e sair']];opts.forEach((o,i)=>button(350,105+i*49,260,39,o[0],pauseIndex===i,o[1]));x.font='12px Arial';x.fillStyle='#b9d4c0';x.fillText((connectedPad()?'D-PAD':'↑ ↓')+' escolher • ← → ajustar • '+confirmHint()+' confirmar • ESC fechar',W/2,516);x.textAlign='left'}
...
function campaignEnd(){uiButtons=[];bg();x.fillStyle='#00140fe8';x.fillRect(0,0,W,H);const sl=activeSlot?readSlot(activeSlot):blankSlot(),records=normalizeLevelRecords(sl.levelRecords,sl.crystals),done=records.filter(r=>r.completed).length,gems=records.filter(r=>r.gem).length,crystals=records.filter(r=>r.crystal).length,percent=Math.round((done+gems+crystals)/27*100);x.textAlign='center';x.fillStyle='#ffe66d';x.font='900 44px Arial';x.fillText('AVENTURA COMPLETA!',W/2,70);x.fillStyle='#dfffe9';x.font='bold 20px Arial';x.fillText('O Guardião das Nove Gemas foi derrotado.',W/2,105);panel(220,126,520,272,.95);x.fillStyle='#fff3bf';x.font='900 23px Arial';x.fillText('PROGRESSO TOTAL  '+percent+'%',W/2,164);x.font='bold 19px Arial';x.fillStyle='#baffc8';x.fillText('✓ FASES CONCLUÍDAS  '+done+'/9',W/2,207);x.fillStyle='#8ff0ff';x.fillText('💎 GEMAS  '+gems+'/9',W/2,246);x.fillText('◆ CRISTAIS  '+crystals+'/9',W/2,285);x.fillStyle='#ffd766';x.font='bold 15px Arial';x.fillText(percent===100?'PERFEITO! VOCÊ DOMINOU TODA A AVENTURA.':'Continue explorando para alcançar 100%.',W/2,326);x.fillStyle='#9ee8bd';x.font='bold 13px Arial';x.fillText('CRIADO E DESENVOLVIDO POR',W/2,356);x.fillStyle='#ffe06b';x.font='900 20px Arial';x.fillText('LUIS PAULO ALVES',W/2,383);button(375,425,210,52,'VOLTAR AO MENU',true,confirmHint());x.textAlign='left'}
function fmtTime(v){let m=Math.floor(v/60),ss=Math.floor(v%60);return String(m).padStart(2,'0')+':'+String(ss).padStart(2,'0')}
function results(){uiButtons=[];bg();x.fillStyle='#00110ed9';x.fillRect(0,0,W,H);x.textAlign='center';x.fillStyle='#ffc83d';x.font='bold 48px Arial';x.fillText('FASE '+campaignNumber()+' COMPLETA!',W/2,64);x.fillStyle=(currentLevel===4||currentLevel===6||currentLevel===7)?'#a9e9ff':currentLevel===3?'#b4db58':'#7ee35a';x.font='bold 24px Arial';x.fillText(currentLevelName,W/2,96);panel(175,112,610,325,.95);x.textAlign='left';x.font='bold 25px Arial';const special=levelMode==='ride'||levelMode==='avalanche',specialState=levelMode==='ride'?rideState:avalancheState,hits=special?(specialState?.hits||0):0,first=special?['COLISÕES',String(hits)]:['CAIXAS',boxesBroken+'/'+(normal.length+tnts.length)];const rows=[first,['FRUTAS',fruits],['MORTES',deaths],['TEMPO',fmtTime(levelTime)],['GEMA',resultGem?'💎 OBTIDA':'🔒 NÃO OBTIDA'],['CRISTAL',crystalsCollected[currentLevel-1]?'🔷 OBTIDO':'◇ NÃO OBTIDO']];rows.forEach((r,i)=>{let yy=154+i*40;x.fillStyle='#ffffff';x.fillText(r[0],235,yy);x.textAlign='right';x.fillStyle=i===4?(resultGem?'#7fe9ff':'#ff7b69'):(i===5?(crystalsCollected[currentLevel-1]?'#7fe9ff':'#b9c7c9'):'#ffe28a');x.fillText(String(r[1]),725,yy);x.textAlign='left'});x.textAlign='center';x.font='bold 18px Arial';x.fillStyle=resultGem?'#7fe9ff':'#ffcf55';const gemHint=special?'Complete a perseguição sem colidir para obter a gema.':'Quebre todas as caixas (incluindo TNT) para obter a gema.';x.fillText(resultGem?'💎 GEMA DE '+currentLevelName:gemHint,W/2,405);if(special){x.fillStyle='#bdeeff';x.font='bold 13px Arial';x.fillText('ESQUIVAS '+(specialState?.nearMisses||0)+'  •  MELHOR COMBO ×'+(specialState?.bestCombo||0),W/2,431)}button(245,455,210,54,'REJOGAR',false,connectedPad()?extraHint():'R');button(505,455,210,54,'CONTINUAR',true,confirmHint());x.textAlign='left'}
...
function loadingScreen(){uiButtons=[];x.fillStyle='#07151d';x.fillRect(0,0,W,H);if(menuLogoImg&&menuLogoImg.complete&&menuLogoImg.naturalWidth){x.globalAlpha=.9;x.drawImage(menuLogoImg,W/2-175,28,350,158);x.globalAlpha=1}const lvl=loadingTargetLevel||currentLevel||1,meta=window.LEVEL_CONFIG?.[lvl],tip=window.LOADING_TIPS?.[lvl]||'Explore com calma e observe o cenário.';x.textAlign='center';x.fillStyle='#ffd45d';x.font='900 30px Arial';x.fillText(loadingText,W/2,228);x.fillStyle='#d9f5ee';x.font='bold 17px Arial';x.fillText(loadingError?'Falha no carregamento':'Carregando...',W/2,262);x.fillStyle='#102a31';x.fillRect(190,286,580,30);x.strokeStyle=loadingError?'#e66d68':'#73d7c2';x.lineWidth=2;x.strokeRect(190,286,580,30);x.fillStyle=loadingError?'#d56b66':'#75d9c4';x.fillRect(194,290,572*Math.max(0,Math.min(1,loadingProgress)),22);x.fillStyle='#f0fff9';x.font='bold 16px Arial';x.fillText(Math.round(loadingProgress*100)+'%',W/2,342);const ang=performance.now()/180;x.save();x.translate(W/2,376);x.rotate(ang);x.strokeStyle='#ffd45d';x.lineWidth=5;x.beginPath();x.arc(0,0,14,.2,4.7);x.stroke();x.restore();x.fillStyle='#a9c9be';x.font='13px Arial';x.fillText('DICA: '+tip,W/2,415);x.fillStyle='#789c92';x.font='12px Arial';x.fillText('CORE + compartilhados permanecem • somente a fase atual fica ativa',W/2,442);if(loadingError){x.fillStyle='#ff9292';x.font='bold 14px Arial';x.fillText(loadingError,W/2,470);if(loadingErrorDetails){x.fillStyle='#c98c8c';x.font='11px Arial';x.fillText(loadingErrorDetails.slice(0,105),W/2,490)}button(370,502,220,34,'TENTAR NOVAMENTE',true,'ENTER')};x.textAlign='left'}
...
window.__crashDebug={
 ...
 getSpecialAssets:()=>({snowball:!!snowballImg,bearRide:!!bearRideImg,boulder:!!boulderImg,boarRide:!!boarRideImg,roadObstacles:!!roadObstaclesImg,soul:!!soulImg}),
