...
const p={x:80,y:groundY-78,w:46,h:78,standH:78,crouchH:48,vx:0,vy:0,on:false,facing:1,spin:0,anim:0,land:0,takeoff:0,crouch:false,slide:0,slideDir:1};
const boxFx=[]; const dustFx=[]; let forcedDeathT=0,swingClock=0;
...
function setLevel(n=1){
 ...
 if(n===5){
   ...
   [350,520,860,1060,1460,1660,2140,2310,2720,2920,3110,3470,3650,4140,4330,4560,5000,5200].forEach((px,i)=>pushBox(px,groundY-70,i%4===0));
   ...
   [340,510,880,1060,1510,1690,2150,2350,2790,3000,3180,3470,3660,4200,4400,4920,5140].forEach((px,i)=>pushBox(px,groundY-70,i%3===0));
...
function resetP(){p.h=p.standH;p.x=checkpoint;p.y=groundY-p.h;p.vx=p.vy=0;p.spin=0;p.slide=0;p.crouch=false;p.land=0;p.takeoff=0;forcedDeathT=0;inv=1.5}
...
function update(dt){if(state!=='play'||paused)return;swingClock+=dt;levelTime+=dt;if(inv>0)inv-=dt;if(toastT>0)toastT-=dt;if(shake>0)shake-=dt;if(deathAnim>0){deathAnim-=dt;if(deathAnim<=0&&lives>0)resetP();return}if(portalSeq>0){portalSeq+=dt;p.vx=0;p.vy=0;const cx=portal.x+portal.w/2-p.w/2,cy=portal.y+portal.h/2-p.h/2;p.x+=(cx-p.x)*Math.min(1,dt*3.6);p.y+=(cy-p.y)*Math.min(1,dt*3.6);if(portalSeq>2.15){resultGem=boxesBroken>=normal.length+tnts.length;if(currentLevel<5)assetManager.preloadPhase(currentLevel+1);if(currentLevel===3)unlockTrophy('pantano');if(currentLevel===4)unlockTrophy('gelo');if(currentLevel===5)unlockTrophy('canyon');state='results';if(fruits>=50)unlockTrophy('frutas50');if(resultGem){unlockTrophy('caixas');unlockTrophy('gema')}if(deaths===0)unlockTrophy('semMorrer');if(levelTime<150)unlockTrophy('veloz');saveGame(true)}return}if(forcedDeathT>0){forcedDeathT-=dt;return}if(p.land>0)p.land-=dt;if(p.takeoff>0)p.takeoff-=dt;for(let i=dustFx.length-1;i>=0;i--){let d=dustFx[i];d.t-=dt;d.x+=d.vx*dt;d.y+=d.vy*dt;d.vy+=220*dt;if(d.t<=0)dustFx.splice(i,1)}for(let i=boxFx.length-1;i>=0;i--){let f=boxFx[i];f.t-=dt;if(f.piece){f.x+=f.vx*dt;f.y+=f.vy*dt;f.vy+=600*dt;f.r+=f.vr*dt}if(f.t<=0)boxFx.splice(i,1)}
 ...
 for(const b of normal){if(b.hit)continue;const br={x:b.x,y:b.y,w:b.w,h:b.h};if(rect(pr,br)){const stomp=p.vy>0&&p.y+p.h-b.y<34;if(p.spin>0||p.slide>0){breakBox(b,p.slide>0?'slide':'spin')}else if(stomp){breakBox(b,'stomp');p.y=b.y-p.h;p.vy=-285;p.on=false;p.land=.1}else if(p.vy<0){p.y=b.y+b.h;p.vy=70}else if(p.vx>0)p.x=b.x-p.w;else if(p.vx<0)p.x=b.x+b.w}}
 for(const t of tnts){if(t.dead){if(t.blast>0)t.blast-=dt;continue}if(t.active){t.t-=dt;if(t.t<=0)explode(t)}const tr={x:t.x,y:t.y,w:t.w,h:t.h};if(rect(pr,tr)){const stomp=p.vy>0&&p.y+p.h-t.y<38;if(p.spin>0||p.slide>0){explode(t);forceDeath();return}else if(stomp){p.y=t.y-p.h;p.vy=-365;p.on=false;if(!t.active){t.active=true;t.t=3.0;toast='TNT: 3...';toastT=.9}puff(p.x+31,t.y,5)}else if(p.vx>0)p.x=t.x-p.w;else if(p.vx<0)p.x=t.x+t.w}}
 ...
 for(const e of enemies){if(e.dead)continue;if(e.hitT>0)e.hitT-=dt;let speedMul=1;if(e.type==='armadillo')speedMul=(Math.sin(swingClock*4.2+e.phase)>0.45?1.65:1);if(e.type==='magmaBeetle')speedMul=(Math.sin(swingClock*3.5+e.phase)>0?1.35:.62);e.x+=e.v*dt*speedMul;if(e.x<e.a||e.x>e.b){e.v*=-1;e.x=Math.max(e.a,Math.min(e.b,e.x))}if(e.type==='mosquito'||e.type==='emberBat')e.y=e.baseY+Math.sin(swingClock*(e.type==='emberBat'?2.7:3.1)+e.phase)*(e.type==='emberBat'?38:28);else if(e.type==='frog')e.y=e.baseY-Math.max(0,Math.sin(swingClock*2.4+e.phase))*34;else e.y=e.baseY;const er={x:e.x,y:e.y,w:e.w||70,h:e.h||56};
   ...
   if(rect(pr,er)){const stomp=p.vy>0&&p.y+p.h<e.y+Math.min(34,(e.h||56)*.65);if(p.spin>0||p.slide>0||(stomp&&e.type!=='magmaBeetle')){e.dead=true;e.hitT=.5;score+=250;toast=(e.type==='mosquito'?'Mosquito':e.type==='frog'?'Sapo':e.type==='penguin'?'Pinguim':e.type==='seal'?'Foca':e.type==='iceShell'?'Casco de gelo':e.type==='emberBat'?'Morcego de brasa':e.type==='swampTurtle'?'Tartaruga do pântano':e.type==='armadillo'?'Tatu':e.type==='magmaBeetle'?'Besouro de magma':e.type==='emberBat'?'Morcego de brasa':'Tartaruga')+' derrotado!';toastT=1;if(stomp){p.vy=-360;p.on=false;puff(p.x+31,e.y,6)}}else lose()}}
...
function drawEnemy(e){let img=turtleImg,fr=turtleSR.walk[Math.floor(swingClock*8)%turtleSR.walk.length],dw=88,dh=62;if(e.type==='magmaBeetle'&&magmaBeetleImg){ds(magmaBeetleImg,null,e.x-30,e.y-72,112,112,e.v<0);return}else if(e.type==='emberBat'&&emberBatImg){ds(emberBatImg,null,e.x-30,e.y-75,110,110,e.v<0);return}else if(e.type==='armadillo'&&armadilloImg){img=armadilloImg;const frames=enemySR.armadillo;fr=frames[0];dw=94;dh=44}else if(e.type!=='turtle'&&biomeEnemyImg){img=biomeEnemyImg;const frames=enemySR[e.type]||enemySR.frog;fr=frames[Math.floor(swingClock*(e.type==='mosquito'?11:7)+e.phase)%frames.length];const sz={swampTurtle:[96,70],frog:[76,62],mosquito:[78,62],penguin:[70,76],seal:[92,66],iceShell:[82,62]}[e.type]||[82,62];dw=sz[0];dh=sz[1]}ds(img,fr,e.x-(dw-(e.w||70))/2,e.y+(e.h||56)-dh+5,dw,dh,e.v<0);}
function draw(){bg();x.save();let sx=shake>0?(Math.random()-.5)*12:0,sy=shake>0?(Math.random()-.5)*8:0;x.translate(-camX+sx,sy);
 ...
 for(const f of boxFx){if(f.piece){x.save();x.translate(f.x,f.y);x.rotate(f.r);x.globalAlpha=Math.max(0,Math.min(1,f.t*2));x.fillStyle='#9a5427';x.fillRect(-f.s,-3,f.s*2,6);x.fillStyle='#d4853d';x.fillRect(-f.s,-3,f.s*1.2,2);x.restore()}else{const a=Math.max(0,f.t/.42),sc=1+(1-a)*.55;x.save();x.globalAlpha=a;x.translate(f.x,f.y);x.rotate((1-a)*(f.mode==='spin'?2.3:.4));x.scale(sc,sc);ds(playerImg,sr[f.q?'qbox':'box'],-29,-29,58,58);x.restore()}}for(const d of dustFx){x.globalAlpha=Math.max(0,d.t*2);x.fillStyle='#e8e0cf';x.beginPath();x.arc(d.x,d.y,d.s,0,7);x.fill();x.globalAlpha=1}for(const e of enemies)if(!e.dead&&visibleWorldX(e.x,e.w||90,220))drawEnemy(e);
 if(checkpoint<2600){ds(checkpointImg,[45,45,220,250],2485,300,125,145)}else{let cpGlow=.55+.35*Math.sin(swingClock*7);x.save();x.globalAlpha=cpGlow;x.fillStyle='#ffd84d';x.beginPath();x.ellipse(2548,430,58,13,0,0,Math.PI*2);x.fill();x.restore();if(checkpointAnim>.8)ds(checkpointImg,[245,380,220,245],2485,300,125,145);else if(checkpointAnim>.35)ds(checkpointImg,[445,350,235,270],2480,285,135,155);else ds(checkpointImg,[720,610,270,270],2475,300,145,145)};
 ...
 let pw=82,ph=112;if(p.land>0){pw=88;ph=104}else if(p.takeoff>0){pw=78;ph=118}const pdx=p.x-(pw-p.w)/2,pdy=p.y+p.h-ph;if(p.spin>0){x.globalAlpha=.14;for(let i=1;i<=3;i++)ds(playerImg,sr[key],pdx-p.vx*.014*i,pdy,82,112,p.facing<0,.14)}
 if(deathAnim<=0){let pa=inv>0&&Math.floor(inv*14)%2?0.35:1,pscale=portalSeq>0?Math.max(.05,1-portalSeq/2.05):1;x.save();x.translate(p.x+p.w/2,p.y+p.h/2);x.scale(pscale,pscale);
   if(p.slide>0){const sf=Math.min(8,Math.floor((.58-p.slide)*17));ds(crouchImg,[sf*280,220,280,220],-64,p.h/2-49,128,96,p.facing<0,pa)}
   else if(p.crouch){const cf=Math.floor(p.anim*1.15)%8;ds(crouchImg,[cf*280,0,280,220],-57,p.h/2-77,114,88,p.facing<0,pa)}
   else ds(playerImg,sr[key],-pw/2,p.h/2-ph+7,pw,ph,p.facing<0,pa*Math.min(1,pscale*1.4));x.restore()}x.globalAlpha=1;
...
function setupTouchControls(){
 ...
 const press=ev=>{const b=ev.target.closest('[data-action]');if(!b)return;ev.preventDefault();const action=b.dataset.action;active.set(ev.pointerId,action);set(action,true);b.setPointerCapture?.(ev.pointerId)};
 const release=ev=>{const action=active.get(ev.pointerId);if(!action)return;ev.preventDefault();active.delete(ev.pointerId);if(![...active.values()].includes(action))set(action,false)};
 root.addEventListener('pointerdown',press,{passive:false});root.addEventListener('pointerup',release,{passive:false});root.addEventListener('pointercancel',release,{passive:false});root.addEventListener('lostpointercapture',release,{passive:false});
 document.addEventListener('visibilitychange',()=>{if(document.hidden){active.clear();for(const k of Object.keys(touchState))touchState[k]=false}});
}
setupTouchControls();
...
function loop(t){let dt=Math.min(.033,(t-last)/1000||0);last=t;gamepadUI();update(dt);if(state==='menu')menu();else if(state==='loading')loadingScreen();else if(state==='play')draw();else if(state==='results')results();else if(state==='levelselect')levelSelect();else if(state==='win')end('FASE CONCLUÍDA!','Você atravessou '+currentLevelName+'!');else end('GAME OVER','Tente novamente e chegue ao fim da fase.');requestAnimationFrame(loop)}
