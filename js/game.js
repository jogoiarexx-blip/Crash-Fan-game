...
class LevelManager{
 ...
 cleanupRuntime(reason='transition'){
  ...
  for(const a of [boxFx,dustFx,enemyFx,impactFx,akuFx,lifePickupFx,akuTrail,rideFx])a.length=0;
...
function blankSlot(){return{empty:true,version:SAVE_VERSION,updated:0,currentLevel:1,phaseName:'ILHA SELVAGEM',checkpoint:80,score:0,fruits:0,phaseStartFruits:0,lives:4,deaths:0,levelTime:0,boxesBroken:0,aku:0,akuInvT:0,rideHits:0,avalancheHits:0,rideNearMisses:0,avalancheNearMisses:0,rideBestCombo:0,avalancheBestCombo:0,rideObstacles:[],avalancheObstacles:[],gem:false,gems:Array(9).fill(false),crystals:Array(9).fill(false),levelRecords:normalizeLevelRecords(),bossDefeated:false,bossHp:9,completed:false,levelsUnlocked:1,normal:[],normalBounce:[],tnts:[],fruitList:[],masks:[],enemies:[],trophies:{}}}
function readSlot(i){
  try{
    ...
    return {
      ...
      masks:arr(parsed.masks),enemies:arr(parsed.enemies),rideBestCombo:num(parsed.rideBestCombo,0),avalancheBestCombo:num(parsed.avalancheBestCombo,0),rideObstacles:arr(parsed.rideObstacles),avalancheObstacles:arr(parsed.avalancheObstacles),
...
const boxFx=[]; const dustFx=[]; const enemyFx=[]; const impactFx=[]; const akuFx=[]; const akuTrail=[]; const lifePickupFx=[]; const rideFx=[]; let forcedDeathT=0,swingClock=0,akuProtectT=0,akuTrailClock=0;
...
const JUNGLE_RIDE_LENGTH=7300;
function rideLength(){return currentLevel===9?JUNGLE_RIDE_LENGTH:RIDE_LENGTH}
const AVALANCHE_LENGTH=6500;
const BOULDER_CHASE_LENGTH=6700;
function avalancheLength(){return currentLevel===8?BOULDER_CHASE_LENGTH:AVALANCHE_LENGTH}
...
function buildRideState(start=0){const jungle=currentLevel===9,pattern=jungle?JUNGLE_RIDE_PATTERN:RIDE_OBSTACLE_PATTERN;return{distance:start,lane:0,targetLane:0,y:0,vy:0,speed:jungle?270:300,hits:0,nearMisses:0,combo:0,bestCombo:0,comboT:0,lean:0,takeoffT:0,landT:0,skidT:0,prevL:false,prevR:false,prevJ:false,obstacles:pattern.map((type,i)=>({type,lane:((i*(jungle?4:5)+i%2)%3)-1,z:520+i*(jungle?290:275),done:false}))}}
...
function restoreSpecialObstacleProgress(state,saved=[],start=0){
 if(!state?.obstacles)return;
 state.obstacles.forEach((o,i)=>{o.done=!!saved?.[i]||o.z<=start+70});
}
...
function setLevel(n=1){
 ...
 if(n===10){
   ...
   */
   ...
   [350,520,860,1060,1460,1660,2140,2310,2720,2920,3110,3470,3400,4140,4330,4560,5000,5200].forEach((px,i)=>pushBox(px,groundY-58,i%4===0));
   pushBox(480,355-58,true);pushBox(1580,355-58,false);pushBox(3115,300-58,true);pushBox(5100,355-58,false);
   [1130,2380,4060,4620].forEach(px=>pushTNT(px));
   ...
   pushEnemy(780,770,810,90,groundY-60,'penguin');pushEnemy(1540,1530,1580,58,groundY-52,'seal');pushEnemy(2210,2200,2250,72,groundY-52,'iceShell');pushEnemy(2840,2810,2890,-96,groundY-60,'penguin');pushEnemy(3540,3530,3600,62,groundY-52,'seal');pushEnemy(4250,4220,4300,76,groundY-52,'iceShell');pushEnemy(5100,5070,5160,-102,groundY-60,'penguin');
 } else if(n===3){
...
function spawnRideFx(kind,px,py,count=6,jungle=false,power=1){for(let i=0;i<count;i++){const a=(Math.random()-.5)*(kind==='skid'?1.7:2.3),spd=(kind==='landing'?40:28)+Math.random()*55*power;rideFx.push({kind,x:px+(Math.random()-.5)*20,y:py+(Math.random()-.5)*10,vx:Math.cos(a)*spd*(kind==='skid'?1.3:.85),vy:(kind==='snow'||kind==='landing'?-35:-20)-Math.random()*65*power,t:.34+Math.random()*.30*power,s:3+Math.random()*6*power,r:Math.random()*6.28,vr:(Math.random()-.5)*4,jungle})}}
function updateRideFx(dt){for(let i=rideFx.length-1;i>=0;i--){const f=rideFx[i];f.t-=dt;f.x+=f.vx*dt;f.y+=f.vy*dt;f.vy+=(f.kind==='leaf'?26:82)*dt;f.vx*=Math.max(.9,1-dt*(f.kind==='leaf'?1.4:3.2));f.r+=f.vr*dt;if(f.t<=0)rideFx.splice(i,1)}}
function drawRideFx(){for(const f of rideFx){const a=Math.max(0,Math.min(1,f.t*2.4));x.save();x.translate(f.x,f.y);x.rotate(f.r);x.globalAlpha=a*(f.kind==='landing'?.95:.8);if(f.kind==='leaf'){x.fillStyle=f.jungle?'#8ac55a':'#e8f6ff';x.beginPath();x.ellipse(0,0,f.s*1.25,f.s*.58,0,0,Math.PI*2);x.fill();x.strokeStyle=f.jungle?'rgba(86,62,25,.45)':'rgba(255,255,255,.45)';x.lineWidth=1;x.beginPath();x.moveTo(-f.s*.9,0);x.lineTo(f.s*.8,0);x.stroke()}else{const cold=!f.jungle;x.fillStyle=cold?(f.kind==='snow'?'#eefcff':'#d9f3ff'):(f.kind==='landing'?'#c99557':'#9cc96a');x.beginPath();x.ellipse(0,0,f.s*(f.kind==='skid'?1.8:1.25),Math.max(1.6,f.s*.58),0,0,Math.PI*2);x.fill();if(f.kind==='landing'){x.globalAlpha*=.55;x.fillStyle=cold?'#ffffff':'#eed4a0';x.fillRect(-f.s*.8,-f.s*.2,f.s*1.6,Math.max(2,f.s*.35))}}x.restore()}}
...
function restartAttemptAfterDeath(){
 ...
 if(hadCheckpoint){
   ...
   const keep={
     score,fruits,levelTime,boxesBroken,aku,akuInvT,rideHits:rideState?.hits||0,avalancheHits:avalancheState?.hits||0,rideNearMisses:rideState?.nearMisses||0,avalancheNearMisses:avalancheState?.nearMisses||0,rideBestCombo:rideState?.bestCombo||0,avalancheBestCombo:avalancheState?.bestCombo||0,rideObstacles:rideState?.obstacles?.map(o=>!!o.done)||[],avalancheObstacles:avalancheState?.obstacles?.map(o=>!!o.done)||[],
   ...
   swingClock=0;
   boxFx.length=0;dustFx.length=0;enemyFx.length=0;impactFx.length=0;akuFx.length=0;lifePickupFx.length=0;rideFx.length=0;akuTrail.length=0;forcedDeathT=0;hudLifePulseT=0;
   prevJ=false;prevS=false;prevDown=false;
   resetP();if(level===6||level===8){checkpoint=3250;avalancheState=buildAvalancheState(3250);avalancheState.hits=keep.avalancheHits;avalancheState.nearMisses=keep.avalancheNearMisses;avalancheState.bestCombo=keep.avalancheBestCombo;restoreSpecialObstacleProgress(avalancheState,keep.avalancheObstacles,3250)}if((level===7||level===9)&&rideState){checkpoint=3600;rideState.distance=3600;rideState.hits=keep.rideHits;rideState.nearMisses=keep.rideNearMisses;rideState.bestCombo=keep.rideBestCombo;restoreSpecialObstacleProgress(rideState,keep.rideObstacles,3600)}camX=Math.max(0,Math.min(worldW-W,p.x-W*.35));
 ...
 boxFx.length=0;dustFx.length=0;enemyFx.length=0;impactFx.length=0;akuFx.length=0;lifePickupFx.length=0;rideFx.length=0;forcedDeathT=0;hudLifePulseT=0;
 prevJ=false;prevS=false;prevDown=false;
...
function updateRide(dt){
 ...
 const prevTarget=rideState.targetLane,lanePx=rideState.lane*125,jungle=currentLevel===9;
 rideState.takeoffT=Math.max(0,(rideState.takeoffT||0)-dt);rideState.landT=Math.max(0,(rideState.landT||0)-dt);rideState.skidT=Math.max(0,(rideState.skidT||0)-dt);
 ...
 if(prevTarget!==rideState.targetLane&&rideState.y<=0){rideState.skidT=.18;spawnRideFx(jungle?'leaf':'snow',W/2+lanePx,474,fxCount(2,4,6),jungle,.9)}
 if(J&&!rideState.prevJ&&rideState.y<=0){rideState.vy=520;rideState.takeoffT=.22;spawnRideFx(jungle?'leaf':'snow',W/2+lanePx,470,fxCount(4,6,9),jungle,1.1);playSfx('jump',1.08,.8,.08)}
 ...
 if(rideState.y>0||rideState.vy>0){rideState.y+=rideState.vy*dt;rideState.vy-=1120*dt;if(rideState.y<=0){rideState.y=0;rideState.vy=0;rideState.landT=.20;spawnRideFx('landing',W/2+rideState.lane*125,476,fxCount(6,10,14),jungle,1.2);playSfx('land',1,.55,.1)}}
 for(const o of rideState.obstacles){if(o.done)continue;const d=o.z-rideState.distance;if(d<-65){if(o.type!=='fruit'&&Math.abs(rideState.lane-o.lane)<.5&&rideState.y>=54){addSpecialChain(rideState,'dodge');spawnRideFx(jungle?'leaf':'snow',W/2+rideState.lane*125+(rideState.lane-o.lane)*18,460,fxCount(2,3,5),jungle,.8)}o.done=true;continue}if(Math.abs(d)<56&&Math.abs(rideState.lane-o.lane)<.44){if(o.type==='fruit'){o.done=true;addWumpas(3);addSpecialChain(rideState,'fruit');playSfx('fruit',1.12,.75,.04)}else if(rideState.y<54){o.done=true;rideState.hits++;rideState.combo=0;rideState.comboT=0;spawnRideFx(jungle?'leaf':'snow',W/2+rideState.lane*125,464,fxCount(7,10,14),jungle,1.3);lose(false,currentLevel===9?'boar':'ride');return}}}
 ...
 if(rideState.distance>=rideLength())finishSpecialLevel(rideState.hits===0);
}
function updateAvalanche(dt){
 ...
 if(!checkpointActivated&&a.distance>3250){checkpointActivated=true;checkpoint=3250;const safeGap=currentLevel===8?720:690;a.gap=Math.max(a.gap,safeGap);toast=currentLevel===8?'CHECKPOINT DA PEDRA!':'CHECKPOINT DA AVALANCHE!';toastT=1.7;score+=250;saveGame(true)}
 ...
 if(a.distance>=avalancheLength())finishSpecialLevel(a.hits===0);
}
...
for(let i=enemyFx.length-1;i>=0;i--){let f=enemyFx[i];f.t-=dt;if(f.t<=0)enemyFx.splice(i,1)}for(let i=impactFx.length-1;i>=0;i--){let f=impactFx[i];f.t-=dt;if(f.t<=0)impactFx.splice(i,1)}for(let i=akuFx.length-1;i>=0;i--){let f=akuFx[i];f.t-=dt;if(f.t<=0)akuFx.splice(i,1)}updateRideFx(dt);if(akuProtectT>0)akuProtectT=Math.max(0,akuProtectT-dt);for(const e of enemies)if(!e.dead&&e.safeT>0)e.safeT=Math.max(0,e.safeT-dt);if(p.stompSquash>0)p.stompSquash=Math.max(0,p.stompSquash-dt);for(const b of normal)if(b.bounceFlash>0)b.bounceFlash=Math.max(0,b.bounceFlash-dt);for(const t of tnts){if(t.dead){if(t.blast>0)t.blast=Math.max(0,t.blast-dt)}else if(t.active){t.t-=dt;const beep=Math.max(1,Math.ceil(t.t));if(beep<t.lastBeep){t.lastBeep=beep;playSfx('tick',1+(3-beep)*.09,.85,.08)}if(t.t<=0)explode(t)}}if(levelMode==='boss'){updateBossLevel(dt);return}if(levelMode==='ride'){updateRide(dt);return}if(levelMode==='avalanche'){updateAvalanche(dt);return}
...
function drawMode7BandMarker(projection,rel,label,color,finish=false,jungle=false){
 if(rel<48||rel>940)return;const mid=projection(0,rel),l=projection(-1.84,rel),r=projection(1.84,rel),l2=projection(-1.84,rel+18),r2=projection(1.84,rel+18),sc=Math.max(.14,mid.scale),poleW=Math.max(3,10*sc),poleH=Math.max(24,120*sc),bannerH=Math.max(10,28*sc),bannerY=mid.y-poleH;
 x.save();x.globalAlpha=Math.min(1,.18+sc*.95);const segments=finish?8:1;
 for(let i=0;i<segments;i++){const t0=i/segments,t1=(i+1)/segments,xa=l.x+(r.x-l.x)*t0,xb=l.x+(r.x-l.x)*t1,xc=l2.x+(r2.x-l2.x)*t1,xd=l2.x+(r2.x-l2.x)*t0;x.beginPath();x.moveTo(xa,l.y);x.lineTo(xb,r.y);x.lineTo(xc,r2.y);x.lineTo(xd,l2.y);x.closePath();x.fillStyle=finish?(i%2===0?'#fff':'#111'):(jungle?'#a2d76c':'#8fdcf0');x.fill()}
 x.globalAlpha=Math.min(1,.24+sc*.9);x.strokeStyle=finish?'rgba(255,225,125,.95)':(jungle?'rgba(226,248,156,.9)':'rgba(225,249,255,.92)');x.lineWidth=Math.max(1,2*sc);x.beginPath();x.moveTo(l.x,l.y);x.lineTo(r.x,r.y);x.lineTo(r2.x,r2.y);x.lineTo(l2.x,l2.y);x.closePath();x.stroke();
 x.globalAlpha=Math.min(1,.35+sc*.9);x.fillStyle=finish?(jungle?'#6d4927':'#5e7988'):(jungle?'#5c3d1c':'#4a6d79');x.fillRect(l.x-poleW*.5,l.y-poleH,poleW,poleH+6*sc);x.fillRect(r.x-poleW*.5,r.y-poleH,poleW,poleH+6*sc);const bx=l.x+poleW*.95,bw=(r.x-l.x)-poleW*1.9;
 if(finish){for(let i=0;i<8;i++){x.fillStyle=i%2===0?'#fff':'#111';x.fillRect(bx+bw*i/8,bannerY,bw/8+1,bannerH)}}else{x.fillStyle=color;x.fillRect(bx,bannerY,bw,bannerH)}
 x.strokeStyle='rgba(255,255,255,.85)';x.lineWidth=Math.max(1,2*sc);x.strokeRect(bx,bannerY,bw,bannerH);if(sc>.28){x.fillStyle=finish?'#ffe89c':'#fff';x.textAlign='center';x.font='900 '+Math.max(10,18*sc)+'px Arial';x.fillText(label,(l.x+r.x)/2,bannerY+bannerH*.73);x.textAlign='left'}x.restore()
}
function drawMode7SpecialMarkers(distance,projection,jungle=false,ride=false){const cp=ride?3600:3250,fin=ride?rideLength():avalancheLength();drawMode7BandMarker(projection,cp-distance,'CHECKPOINT',jungle?'#86c94f':'#69d9ff',false,jungle);drawMode7BandMarker(projection,fin-distance,'CHEGADA',jungle?'#e1a95a':'#ffd567',true,jungle)}
function drawRideSignatureEvents(rs,jungle=false){
 const distance=rs.distance,projection=rideProjection;
 if(jungle){
  for(let id=Math.floor(distance/760)-1;id<Math.floor(distance/760)+3;id++){const rel=520+id*760-distance;if(rel<90||rel>920)continue;const side=id%2===0?-1:1,mid=projection(side*.42,rel),sc=Math.max(.18,mid.scale);x.save();x.globalAlpha=.22+sc*.7;x.strokeStyle='#5b3718';x.lineWidth=Math.max(2,10*sc);const p0=projection(-1.28,rel+8),p1=projection(-.45,rel),p2=projection(.28,rel+14),p3=projection(1.08,rel+4);x.beginPath();x.moveTo(p0.x,p0.y);x.bezierCurveTo(p1.x,p1.y-24*sc,p2.x,p2.y+10*sc,p3.x,p3.y);x.stroke();x.globalAlpha=.18+sc*.45;x.fillStyle='#6a4723';x.beginPath();x.ellipse(mid.x,mid.y-6*sc,26*sc,11*sc,0,0,Math.PI*2);x.fill();for(let i=0;i<fxCount(2,4,6);i++){const ox=(i-2)*10*sc,oy=(i%2)*-8*sc;x.fillStyle=i%2===0?'#b98b57':'#8fbf66';x.beginPath();x.ellipse(mid.x+ox,mid.y-18*sc+oy,5*sc,2.8*sc,Math.sin(i+id),0,Math.PI*2);x.fill()}const edge=projection(side*2.18,rel+14);x.globalAlpha=.24+sc*.52;x.fillStyle='#7e6a48';x.fillRect(edge.x-10*sc,edge.y-34*sc,18*sc,14*sc);x.fillRect(edge.x+4*sc,edge.y-20*sc,12*sc,10*sc);x.restore()}
 }else{
  for(let id=Math.floor(distance/820)-1;id<Math.floor(distance/820)+3;id++){const rel=600+id*820-distance;if(rel<90||rel>920)continue;const side=id%2===0?-1:1,mid=projection(side*.55,rel),sc=Math.max(.18,mid.scale),gust=.55+Math.sin(swingClock*8+id)*.25;x.save();x.globalAlpha=.18+sc*.46;x.strokeStyle='#effdff';x.lineWidth=Math.max(1,3.4*sc);const a=projection(-.9,rel+8),b=projection(-.15,rel),c=projection(.65,rel+12);x.beginPath();x.moveTo(a.x,a.y);x.lineTo(b.x,b.y);x.lineTo(c.x,c.y);x.stroke();x.beginPath();x.moveTo(b.x,b.y);x.lineTo(b.x-18*sc,b.y+14*sc);x.moveTo(b.x,b.y);x.lineTo(b.x+16*sc,b.y+12*sc);x.stroke();x.globalAlpha=.22+sc*.48;x.strokeStyle='rgba(255,255,255,'+(.46+gust*.28)+')';for(let i=0;i<4;i++){const yy=mid.y-(30-i*11)*sc;x.lineWidth=Math.max(1,2.4*sc);x.beginPath();x.moveTo(mid.x-side*(18+i*4)*sc,yy);x.quadraticCurveTo(mid.x-side*42*sc,yy-5*sc,mid.x-side*72*sc,yy+8*sc);x.stroke()}for(let i=0;i<fxCount(2,4,6);i++){x.globalAlpha=.2+sc*.2;x.fillStyle=i%2===0?'#e9fdff':'#c4ecf7';x.beginPath();x.moveTo(mid.x-16*sc+i*9*sc,mid.y-18*sc);x.lineTo(mid.x-8*sc+i*9*sc,mid.y-24*sc);x.lineTo(mid.x-4*sc+i*9*sc,mid.y-14*sc);x.closePath();x.fill()}x.restore()}
 }
}
...
function drawRideLevel(){
 ...
 x.save();if(!reducedMotion&&graphicsTier()>0){x.translate(W/2,H/2);x.rotate(rs.lean*.010);x.translate(-W/2,-H/2)}drawMode7Atmosphere(jungle,0,true);x.fillStyle=jungle?'rgba(42,92,37,.10)':'rgba(218,244,255,.10)';x.fillRect(0,160,W,380);drawMode7Ground(rideProjection,rs.distance,jungle);drawMode7Roadside(rideProjection,rs.distance,jungle);drawMode7SpecialMarkers(rs.distance,rideProjection,jungle,true);drawRideSignatureEvents(rs,jungle);drawRideSurfaceFX(rs,jungle);
 ...
 const laneX=rs.lane*125,air=rs.y*.65,turn=Math.abs(rs.targetLane-rs.lane),runFr=Math.floor(p.anim)%4,frame=rs.y>22||rs.takeoffT>.02?6:rs.landT>.02?7:rs.skidT>.02?(rs.targetLane<rs.lane?4:5):turn>.08?(rs.targetLane<rs.lane?4:5):runFr,mountBob=Math.sin(p.anim*Math.PI*2)*(turn>.04?4.8:2.6)+(rs.skidT>0?Math.sin(rs.skidT*55)*2.2:0),landingSquash=rs.y>0?1.04:(rs.landT>0?(.86+rs.landT*.65):.98+Math.sin(p.anim*Math.PI*2)*.02),jumpTilt=rs.y>0?(-.05-rs.y*.00028):0;
 drawRideFx();
 if(introT>0&&!reducedMotion)drawPolarMountIntro();
 else if(deathAnim>0){const q=(DEATH_DURATION-deathAnim)/DEATH_DURATION,bounce=Math.sin(Math.min(1,q*1.45)*Math.PI)*72;x.save();x.translate(W/2+laneX+q*54,462-air-bounce+q*86);x.rotate(q*3.15);x.scale(1+q*.08,1-q*.12);ds(mount,[7*256,0,256,256],-125,-205,250,250,false,Math.max(.12,1-q*.76));x.restore();x.save();x.fillStyle=jungle?'#cfac6d':'#effcff';for(let i=0;i<fxCount(3,6,9);i++){x.globalAlpha=Math.max(0,.65-q*.55);x.beginPath();x.arc(W/2+laneX-45+i*13-q*55,464+i%2*9,4+i%3*2,0,Math.PI*2);x.fill()}x.restore()}else{drawMode7ShadowEllipse(W/2+laneX,475-air*.08,48+turn*10,11+Math.max(0,10-rs.y*.03),.22,jungle?'#191008':'#17313c');x.save();x.translate(W/2+laneX,380-air+mountBob-(rs.takeoffT>0?rs.takeoffT*18:0));x.rotate((rs.targetLane-rs.lane)*-.08+jumpTilt+(rs.landT>0?(rs.targetLane-rs.lane)*-.04:0));x.scale(1/landingSquash,landingSquash);if(rs.landT>0){x.globalAlpha=.94+.06*Math.sin(rs.landT*30)}ds(mount,[frame*256,0,256,256],-125,-125,250,250);x.restore();}
 drawMode7Climate(rs.distance,jungle,!jungle,.2+Math.min(.35,rs.speed/700));x.restore();
 drawSpecialTopHud(jungle?'CORRIDA DO JAVALI':'CORRIDA POLAR',rs,jungle?'#b7e171':'#8fdcf0',jungle?'#8ccb50':'#70ddf4',Math.min(1,rs.distance/rideLength()),'DISTÂNCIA',0,'IMPACTOS',Math.min(1,(rs.hits||0)/6));
...
function drawAvalancheLevel(){
 ...
 x.save();if(!reducedMotion&&graphicsTier()>0){x.translate(W/2,H/2);x.rotate(a.lean*.011+(danger>.72?Math.sin(swingClock*15)*.003:0));x.translate(-W/2,-H/2)}drawMode7Atmosphere(jungle,danger,false);x.fillStyle=jungle?'rgba(42,92,37,.09)':'rgba(218,244,255,.10)';x.fillRect(0,154,W,H-154);drawMode7Ground(avalancheProjection,a.distance,jungle);drawMode7Roadside(avalancheProjection,a.distance,jungle);drawMode7SpecialMarkers(a.distance,avalancheProjection,jungle,false);
 ...
 drawSpecialTopHud(jungle?'PEDRA DA SELVA':'AVALANCHE ALPINA',a,danger>.68?'#ff795f':jungle?'#9fd56f':'#8fdcf0',danger>.68?'#ff5b4b':jungle?'#8ecb55':'#70ddf4',Math.min(1,a.distance/avalancheLength()),'DISTÂNCIA',danger,jungle?'PEDRA':'AVALANCHE',danger);
...
function startNow(newGame=true){
 ...
 boxFx.length=0;dustFx.length=0;enemyFx.length=0;impactFx.length=0;akuFx.length=0;lifePickupFx.length=0;rideFx.length=0;akuProtectT=0;forcedDeathT=0;hudLifePulseT=0;
...
window.__crashDebug={
 ...
 testRideMode:()=>{state='play';paused=false;introT=0;deathAnim=0;lives=4;checkpointActivated=false;checkpoint=currentCheckpointDefault;rideState=buildRideState();const before=rideState.distance;keys.ArrowRight=true;update(1/60);keys.ArrowRight=false;const movedLane=rideState.targetLane===1&&rideState.lane>0;keys.Space=true;update(1/60);keys.Space=false;for(let i=0;i<12;i++)update(1/60);draw();const jumped=rideState.y>0,advanced=rideState.distance>before;rideState.distance=3599;update(.1);const checkpointReached=checkpointActivated&&checkpoint===3600;rideState=buildRideState(rideLength()-1);checkpointActivated=true;checkpoint=3600;update(.1);const completed=state==='results',perfect=resultGem;return{mode:levelMode,movedLane,jumped,advanced,checkpointReached,completed,perfect}},
 testRideCheckpointPersistence:()=>{const original={score,fruits,lives,deaths,levelTime,checkpoint,checkpointActivated,state,paused,introT,deathAnim,aku,inv};state='play';paused=false;introT=0;deathAnim=0;lives=4;deaths=0;levelTime=18;aku=0;inv=0;checkpointActivated=true;checkpoint=3600;score=1100;fruits=31;const jungle=currentLevel===9,preIndex=jungle?10:11,fruitIndex=jungle?16:17,crateIndex=jungle?18:14;rideState=buildRideState(3600);const initial={speed:rideState.speed,length:rideLength(),lastObstacleZ:rideState.obstacles.at(-1)?.z||0};rideState.hits=1;rideState.nearMisses=5;rideState.bestCombo=8;rideState.obstacles[crateIndex].done=true;rideState.obstacles[fruitIndex].done=true;restartAttemptAfterDeath();const afterRestart={distance:rideState.distance,score,fruits,hits:rideState.hits,nearMisses:rideState.nearMisses,bestCombo:rideState.bestCombo,preCheckpointDone:rideState.obstacles[preIndex].done,fruitDone:rideState.obstacles[fruitIndex].done,crateDone:rideState.obstacles[crateIndex].done};const saved=buildSave(false);rideState.obstacles[crateIndex].done=false;rideState.obstacles[fruitIndex].done=false;rideState.bestCombo=0;applySaveData(0,saved);const afterLoad={distance:rideState.distance,bestCombo:rideState.bestCombo,preCheckpointDone:rideState.obstacles[preIndex].done,fruitDone:rideState.obstacles[fruitIndex].done,crateDone:rideState.obstacles[crateIndex].done,savedBestCombo:saved.rideBestCombo,savedObstacleCount:saved.rideObstacles.length};score=original.score;fruits=original.fruits;lives=original.lives;deaths=original.deaths;levelTime=original.levelTime;checkpoint=original.checkpoint;checkpointActivated=original.checkpointActivated;state=original.state;paused=original.paused;introT=original.introT;deathAnim=original.deathAnim;aku=original.aku;inv=original.inv;return{obstacles:rideState?.obstacles?.length||0,initial,afterRestart,afterLoad}},
 ...
 testAvalanche:()=>{state='play';paused=false;introT=0;deathAnim=0;lives=4;aku=0;inv=0;checkpointActivated=false;avalancheState=buildAvalancheState();const before=avalancheState.distance;keys.ArrowRight=true;update(1/60);keys.ArrowRight=false;const movedLane=avalancheState.targetLane===1&&avalancheState.lane>0;keys.Space=true;update(1/60);keys.Space=false;for(let i=0;i<8;i++)update(1/60);draw();const jumped=avalancheState.y>0,advanced=avalancheState.distance>before;avalancheState.distance=3249;update(.1);const checkpointReached=checkpointActivated&&checkpoint===3250;deathAnim=0;state='play';lives=4;inv=0;avalancheState.gap=169;update(1/60);const expectedKind=currentLevel===8?'boulder':'snowball',specificDeath=deathAnim>0&&deathKind===expectedKind&&lives===3;deathAnim=0;state='play';lives=4;checkpointActivated=true;avalancheState=buildAvalancheState(avalancheLength()-1);avalancheState.gap=690;update(.1);const completed=state==='results',perfect=resultGem;return{mode:levelMode,movedLane,jumped,advanced,checkpointReached,specificDeath,kind:deathKind,completed,perfect}},
 testAvalancheCheckpointPersistence:()=>{const original={score,fruits,lives,deaths,levelTime,checkpoint,checkpointActivated,state,paused,introT,deathAnim,aku,inv};state='play';paused=false;introT=0;deathAnim=0;lives=4;deaths=0;levelTime=12;aku=0;inv=0;checkpointActivated=true;checkpoint=3250;score=900;fruits=23;const jungle=currentLevel===8,preIndex=jungle?9:10,fruitIndex=jungle?17:13,rockIndex=jungle?19:15;avalancheState=buildAvalancheState(3250);const initial={speed:avalancheState.speed,gap:avalancheState.gap,length:avalancheLength(),lastObstacleZ:avalancheState.obstacles.at(-1)?.z||0};avalancheState.hits=2;avalancheState.nearMisses=4;avalancheState.bestCombo=7;avalancheState.obstacles[fruitIndex].done=true;avalancheState.obstacles[rockIndex].done=true;restartAttemptAfterDeath();const afterRestart={distance:avalancheState.distance,score,fruits,hits:avalancheState.hits,nearMisses:avalancheState.nearMisses,bestCombo:avalancheState.bestCombo,gap:avalancheState.gap,preCheckpointDone:avalancheState.obstacles[preIndex].done,fruitDone:avalancheState.obstacles[fruitIndex].done,rockDone:avalancheState.obstacles[rockIndex].done};const saved=buildSave(false);avalancheState.obstacles[fruitIndex].done=false;avalancheState.obstacles[rockIndex].done=false;avalancheState.bestCombo=0;applySaveData(0,saved);const afterLoad={distance:avalancheState.distance,bestCombo:avalancheState.bestCombo,gap:avalancheState.gap,preCheckpointDone:avalancheState.obstacles[preIndex].done,fruitDone:avalancheState.obstacles[fruitIndex].done,rockDone:avalancheState.obstacles[rockIndex].done,savedBestCombo:saved.avalancheBestCombo,savedObstacleCount:saved.avalancheObstacles.length};score=original.score;fruits=original.fruits;lives=original.lives;deaths=original.deaths;levelTime=original.levelTime;checkpoint=original.checkpoint;checkpointActivated=original.checkpointActivated;state=original.state;paused=original.paused;introT=original.introT;deathAnim=original.deathAnim;aku=original.aku;inv=original.inv;return{obstacles:avalancheState?.obstacles?.length||0,initial,afterRestart,afterLoad}},
 ...
 testBossPersistence:()=>{if(!bossState)return null;state='play';paused=false;deathAnim=0;introT=0;checkpointActivated=false;checkpoint=currentCheckpointDefault;score=2222;fruits=17;phaseStartFruits=17;lives=3;deaths=1;levelTime=44;bossState=window.FinalBoss.create();bossState.intro=0;bossState.hp=5;bossState.phase=2;const saved=buildSave(false);bossState=window.FinalBoss.create();applySaveData(0,saved);const loaded={hp:bossState.hp,phase:bossState.phase,intro:bossState.intro,state,score,fruits,lives};state='play';paused=false;deathAnim=0;checkpointActivated=false;checkpoint=currentCheckpointDefault;bossState=window.FinalBoss.create();bossState.intro=0;bossState.hp=2;bossState.phase=3;score=333;fruits=17;phaseStartFruits=17;levelTime=18;restartAttemptAfterDeath();const restarted={hp:bossState.hp,phase:bossState.phase,intro:bossState.intro,score,fruits,checkpoint,checkpointActivated,state};return{savedHp:saved.bossHp,savedDefeated:saved.bossDefeated,loaded,restarted}},
